...
\bfseries Description & Blend with variable indexes. The first input vector contains the indexes. The following two input vectors contain the data sources. Each index must be in the range  $0 \leq i \leq 2\cdot n - 1$ where n is the number of elements in each vector. \\ \hline
\bfseries Efficiency & 
...
\bfseries Description & Blend with variable indexes. The first input vector contains the indexes. The following four input vectors contain the data sources. Each index must be in the range  $0 \leq i \leq 4\cdot n - 1$ where n is the number of elements in each vector. \\ \hline
\bfseries Efficiency & 
...
\bfseries Defined for & Vec8h, Vec4f, Vec2d \\ \hline
...
Vec32s lookup\textless n\textgreater(Vec32s index, void const * table) \newline
...
Vec8h lookup\textless n\textgreater(Vec8s index, Float16 const * table) \newline
Vec16h lookup\textless n\textgreater(Vec16s index, Float16 const * table) \newline
Vec32h lookup\textless n\textgreater(Vec32s index, Float16 const * table) \newline
...
\bfseries Defined for & floating point and signed integer vector classes listed above \\ \hline
...
\bfseries Efficiency & depends on the instruction set and n.
This is implemented with permute instructions if n is no bigger than twice the vector length and with gather instructions if n is large. It is inefficient if AVX2 is not supported. \\ \hline
\end{tabular}
\vspacebig
...
The index is always an integer vector. The input can be one or more vectors or an array. The result type is determined by the inputs. All elements in the index vector must be in the specified range. The behavior for an index out of range is implementation-dependent and may give any value for the corresponding element. The function may in some cases read past the end of the table up to a total size of two or four vector lengths for the sake of efficient permutation.
\vspacesmall

The lookup functions are not defined for unsigned integer vector types, but the corresponding signed versions can be used. You do not have to worry about overflow when converting unsigned integers to signed here, as long as the result vector is converted back to unsigned.
\vspacebig
