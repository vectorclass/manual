...
This compiler produced the best optimized code in my tests. The Clang compiler is available for all x86 and x86-64 platforms. There are different versions of Clang available for the Windows platform. The Clang version that comes with Cygwin64 should be avoided because it is using an inefficient memory model. You may get Clang directly from \url{https://releases.llvm.org/} or you may use the plugin for Visual Studion (version 16.3 or later) or any other IDE.
\vspacesmall
