...
Older versions of the C++ standard do not specify how to make aligned memory allocation. C++ version 17 supports alignment with \codei{std::aligned\_alloc}. Unfortunately, this does not yet work with all compilers.
\vspacesmall

A method that seems to work with current compilers is to align an array of vectors with operator \codei{new}. The compiler recognizes that a vector class, e.g. \codei{Vec8f}, needs alignment. An array of such vectors will be aligned correctly, even when allocated with \codei{new}. This is illustrated in the following example:
