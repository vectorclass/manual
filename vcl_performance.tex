...
The current version of Intel C++ compiler (version 19.0) cannot compile version 2.xx of VCL due to problems with \codei{constexpr} functions. Use version 1.xx of VCL instead with Intel compiler.\\
...
Older versions of the C++ standard do not specify how to make aligned memory allocation. Newer versions of C++ support alignment with \codei{std::align} (C++11) or \codei{std::aligned\_alloc} (C++17). Linux systems have the function \codei{posix\_memalign} but this method may not be portable to all platforms.
\vspacesmall
...
\item Use \codei{\_\_fastcall} in 64-bit Windows. The Clang and Microsoft compilers can transfer vector parameters and vector returns in registers when \codei{\_\_fastcall} is used on the function declaration. See the example on page \pageref{examplePolynomialVectorcall}.
